#!/bin/bash
# /opt/bin/vasp-motd.sh
# VASP-specific Message of the Day.
# Displays container identity, hardware check, and runtime hints.
#
# Build-time ENV variables (set in Dockerfile):
#   HPC_CONTAINER_NAME     — e.g. vasp-mkl
#   HPC_CONTAINER_TAG      — e.g. 6.6.0
...
# ── VASP environment ─────────────────────────────────────────────────────
if command -v vasp_std &>/dev/null; then
    VASP_EXEC="vasp_std"
else
    VASP_EXEC="vasp (not found in PATH)"
fi
...
echo "  ${BLD}Executable${RST}: ${DIM}${VASP_EXEC}${RST}"
echo ""
echo "  ${B_YLW}HINT:${RST}"
echo "  To optimize performance, use:"
echo "  ${GRN}mpirun "'$MPI_RUNVAR'" -np <N> -x OMP_NUM_THREADS=x vasp_std${RST}"
if [[ -n "${MPI_AMDRUNVAR:-}" ]]; then
    echo "  For AMD CPUs: mpirun "'$MPI_AMDRUNVAR'" (adds libfakeintel)."
fi
echo "  ${DIM}Customize: ${CYA}echo "'$MPI_RUNVAR'"${RST} ${DIM}to inspect, or override in your launch script.${RST}"
echo "  ${DIM}If you encountered shmem issue, please add ${CYA}\"--mca btl_sm_backing_directory /tmp\"${RST} ${DIM}in mpirun command to bypass it. ${RST}"

echo " ${DIM}${LINE}${RST}"
echo "  Type ${CYA}'vasp_std -v'${RST} for more details."
echo "  ${CYA}'vasp_gam, vasp_ncl'${RST} is also available."
echo " ${DIM}${LINE}${RST}"
