...
def test_cp2k_2026_2_declares_image_only_avx512_repo() -> None:
    env_dir = SPACK_ENVS_DIR / "cp2k_opensource-2026.2-force-avx512"
    if not (env_dir / "spack-env-file" / "env.yaml").exists():
        pytest.skip("env missing")
    spec = load_environment_spec(env_dir)
    if not any(r.namespace == "s8_overrides" for r in spec.spack.custom_repos):
        pytest.skip(
            "2026.2 image-only s8_overrides app config not present (parked work)"
        )
    plan = build_spack_environment_plan(spec)
    assets_ns = [r.namespace for r in plan.assets.repos]
    image_ns = [r.namespace for r in plan.image.repos]
    assert "s8_overrides" not in assets_ns
