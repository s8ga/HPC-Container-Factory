...
echo " ${DIM}${LINE}${RST}"
echo "  ${B_RED}⚠  WARNING:${RST} ${RED}This build is HIGHLY EXPERIMENTAL!${RST}"
echo "              ${RED}ONLY cp2k's regtest verified. Use with caution.${RST}"
echo ""
...
echo "  ${GRN}mpirun "'$MPI_RUNVAR'" -np <N> -x OMP_NUM_THREADS=x cp2k.psmp -i <Your Input>.inp${RST}"
if [[ -n "${MPI_AMDRUNVAR:-}" ]]; then
    echo "  ${DIM}For AMD CPUs: ${CYA}mpirun "'$MPI_AMDRUNVAR'"${RST} ${DIM}(adds libfakeintel).${RST}"
fi
