    ...
    def cmake_args(self):
        ...
        if "^blas" in spec:
            if spec["blas"].name == "intel-oneapi-mkl":
                mkl_spec = spec["intel-oneapi-mkl"]
                args.append(self.define("MKL_ROOT", mkl_spec.prefix.mkl.latest))
            
                if mkl_spec.satisfies("threads=openmp"):
                    if spec.satisfies("%gcc"):
                        args.append(self.define("MKL_THREADING", "gnu_thread"))
                    else:
                        args.append(self.define("MKL_THREADING", "intel_thread"))
                elif mkl_spec.satisfies("threads=tbb"):
                    args.append(self.define("MKL_THREADING", "tbb_thread"))
                else:
                    args.append(self.define("MKL_THREADING", "sequential"))

                if mkl_spec.satisfies("mpi_family=openmpi"):
                    args.append(self.define("MKL_MPI", "openmpi"))
                elif mkl_spec.satisfies("mpi_family=mpich"):
                    args.append(self.define("MKL_MPI", "mpich"))

                if mkl_spec.satisfies("+ilp64"):
                    args.append(self.define("MKL_INTERFACE", "ilp64"))
                else:
                    args.append(self.define("MKL_INTERFACE", "lp64"))       
        return args