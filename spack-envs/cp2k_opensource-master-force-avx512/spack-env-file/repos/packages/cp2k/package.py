...
class Cp2k(MakefilePackage, CMakePackage, CudaPackage, ROCmPackage):
    ...
    with when("+libint"):
        ...
        for lmax in HFX_LMAX_RANGE:
            # The CP2K dev repo encodes lmax in the libint version.
            depends_on(
    ...
    patch("posix_c_source.patch", when="@7.1:2026.2 %aocc@4.0:")
    patch("posix_c_source.patch", when="@7.1:2026.2 %gcc@13:")
    ...
    patch("cmake-relwithdebinfo-2024.1.patch", when="@2024.1:2026.2 build_system=cmake")

    # Bugfix for D4 dispersion correction in CP2K 2024.3
    # https://github.com/cp2k/cp2k/issues/3688
    patch("d4-dispersion-bugfix-2024.3.patch", when="@2024.3:2026.2")
